...
const common = {
   ...
   setToSecureStorage(key, object) {
      return new Promise((resolve, reject) => {
         reactLocalStorage.set(key, object).then(
            (res) => {
               resolve(res)
            },
            (err) => {
   ...
   getFromSecureStorage(key) {
      return new Promise((resolve, reject) => {
         reactLocalStorage.get(key).then(
            (res) => {
               resolve(res)
            },
            (err) => {
   ...
   removeFromSecureStorage(key) {
      return new Promise((resolve, reject) => {
         reactLocalStorage.remove(key).then(
            (res) => {
   ...
   removeDuplicates(array, property) {
      ...
      return array.filter(
         (obj) =>
            !uniqueMap.has(obj[property]) && uniqueMap.set(obj[property], 1)
      )
   },
   listCountry() {
      let country = [
